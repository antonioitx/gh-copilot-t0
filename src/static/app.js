    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        if (details.participants.length > 0) {
          participantsHtml = `<ul class="participants-list no-bullets">${details.participants.map(participant => `
            <li>
              <span class="participant-email">${participant}</span>
              <span class="delete-participant" title="Eliminar" data-activity="${name}" data-email="${participant}">&#128465;</span>
            </li>`).join('')}</ul>`;
        } else {
